...
{
    KApplication::kApplication()->invokeHelp("mpegencoder", "kipi-plugins");
}
...
void KImg2mpgData::writeSettings()
{
    ...
    config->writeEntry("VideoFormat", m_VideoFormatConfig);

    m_VideoTypeConfig = m_VideoTypeComboBox->currentText();
    config->writeEntry("VideoType", m_VideoTypeConfig);

    m_ImageDurationConfig = m_DurationImageSpinBox->text();
    config->writeEntry("ImageDuration", m_ImageDurationConfig);
    
    m_TransitionSpeedConfig = m_TransitionComboBox->currentText();
    config->writeEntry("TransitionSpeed", m_TransitionSpeedConfig);
    
    m_BackgroundColorConfig = m_BackgroundColorButton->color();
    config->writeEntry("BackgroundColor", m_BackgroundColorConfig);
    
    m_AudioInputFileConfig = m_AudioInputEDITFilename->text();
    config->writePathEntry("AudioInputFile", m_AudioInputFileConfig);
    
    m_MPEGOutputFileConfig = m_MPEGOutputEDITFilename->text();
    config->writePathEntry("MPEGOutputFile", m_MPEGOutputFileConfig);
    
    config->writePathEntry("ImageMagickBinFolder", m_IMBinFolderConfig);
    config->writePathEntry("MjpegToolsBinFolder", m_MJBinFolderConfig);
...
void KImg2mpgData::readSettings()
{
    ...
    m_VideoFormatConfig = config->readEntry("VideoFormat", "XVCD");
    ...
    m_VideoTypeConfig = config->readEntry("VideoType", "PAL");
    ...
    m_ImageDurationConfig = config->readEntry("ImageDuration", "10");
    m_DurationImageSpinBox->setValue(m_ImageDurationConfig.toInt());
    
    m_TransitionSpeedConfig = config->readEntry("TransitionSpeed", m_NoneLabel);
    ...
    m_BackgroundColorConfig = config->readColorEntry("BackgroundColor", ColorB);
    m_BackgroundColorButton->setColor(m_BackgroundColorConfig);
    
    m_AudioInputFileConfig = config->readPathEntry("AudioInputFile");
    m_AudioInputEDITFilename->setText(m_AudioInputFileConfig);
    
    m_MPEGOutputFileConfig = config->readPathEntry("MPEGOutputFile", KGlobalSettings::documentPath() + "output.mpg");
    m_MPEGOutputEDITFilename->setText(m_MPEGOutputFileConfig);
    
    m_IMBinFolderConfig = config->readPathEntry("ImageMagickBinFolder", "/usr/bin");
    m_MJBinFolderConfig = config->readPathEntry("MjpegToolsBinFolder", "/usr/bin");
