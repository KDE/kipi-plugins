    ...
    : QGraphicsView(parent), d(new Private)
{
    ...
    connect(d->zoomInAction, &QAction::triggered, this, &KPPreviewImage::slotZoomIn);
    ...
    connect(d->zoomOutAction, &QAction::triggered, this, &KPPreviewImage::slotZoomOut);
    ...
    connect(d->zoom2FitAction, &QAction::triggered, this, &KPPreviewImage::slotZoom2Fit);
...
QRectF KPPreviewImage::getSelectionArea() const
{
    return d->selection->rect();
}

void KPPreviewImage::setSelectionArea(const QRectF& rectangle)
{
...
bool KPPreviewImage::load(const QUrl& file) const
{
    ...
    {
        qCDebug(KIPIPLUGINS_LOG) << d->scene->height() << " " << d->scene->width();
